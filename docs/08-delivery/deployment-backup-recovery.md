...
| ระยะเวลา | [รอยืนยัน] |
| สิ่งที่ติดตามทุกวัน | `CAPTURE_RATE` · `OUTCOME_COMPLETION` · `DUPLICATE_RATE` (< 2%) · `MISSING_REQUIRED_RATE` (< 2%) (ข้อ 12.3) · จำนวน `VISIT_UNRECORDED` · ข้อผิดพลาดที่ผู้ใช้แจ้ง · error rate RPC · เหตุการณ์ล็อกบัญชี |
| **เกณฑ์ที่ใช้ตัดสินระหว่าง pilot** | **ใช้ CANONICAL ข้อ 20.10 ไม่ใช่เป้าของข้อ 12.3** — `CAPTURE_RATE` ≥ **80%** ช่วงทดลองคู่กับกระดาษ · ≥ **90%** เมื่อใช้จริงเต็มสาขา · บันทึกผล visit ≥ **90%** · ส่วนต่างจากการนับมือ < 5% · ไม่มี blocker ค้าง<br>ข้อ 12.3 (95%) คือ**เป้าขององค์กรหลังระบบและพนักงานนิ่งแล้ว** และเป็นค่าที่ `target_met` ของ `api.get_kpis` ใช้ — หน้าจอจึงขึ้น "ต่ำกว่าเป้า" ได้ตามปกติระหว่าง pilot **โดยที่ pilot ยังผ่านเกณฑ์ข้อ 20.10** (Direction มีผลบังคับสูงสุด) |
...
| AL-15 | **429 และ Auth failure ของ Auth endpoint** — เก็บ **แยกตาม endpoint** (`/auth/v1/token` · `/auth/v1/verify` · `/auth/v1/recover` · `/auth/v1/invite` · `/functions/v1/staff-code-login` · `/functions/v1/password-reset`) | มี 429 จากการใช้งานจริง (ไม่ใช่การทดสอบ) | กลาง | ผู้ดูแลระบบ · BUSINESS_ADMIN | ดูว่าชนที่ endpoint ไหนและเพราะอะไร → **ปรับค่าเฉพาะตัวนั้น** · **ห้ามเพิ่มเพดานล่วงหน้าเพื่อกันไว้ก่อน** เพราะเพดานที่สูงเกินจำเป็นคือช่องให้เดารหัสผ่าน | CANONICAL ข้อ 20.21 (C-9) |
