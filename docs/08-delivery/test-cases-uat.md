...
|---|---|---|---|
| ~~G1~~ | ~~`tools/check-canonical.mjs` ยังไม่มีไฟล์~~ | — | **ปิดแล้ว (23 ก.ย. 2569)** — ไฟล์มีจริงและมี 14 ข้อตรวจ · `npm run check` รันครบทั้งสี่ขั้นและผ่าน |
...
| G3 | ชั้น **T8** (เบราว์เซอร์อัตโนมัติ) ยังไม่มีสคริปต์ | flow รายบทบาทยังต้องทดสอบด้วยมือใน UAT | เขียนหลัง Pilot เมื่อรู้แล้วว่า flow ไหนเปลี่ยนบ่อย — เขียนก่อนจะต้องตามแก้ทุกครั้งที่หน้าจอขยับ |
| G3b | ชั้น **T7** (HTTP-level บน environment จริง) — **ปิดบางส่วนแล้ว** | `tools/smoke-test.py --env <ไฟล์.json>` ยิงผ่าน HTTP ของแอปจริงด้วยเซสชันจริง ครอบเส้นทางหลักและตรวจว่าตัวเลขบนหน้าจอตรงกับ RPC + ไม่มี PII หลุด | ที่ยังไม่ครอบ: column grant ผ่าน HTTP · `max_rows` · `rpc/svc_*` ต้องถูกปฏิเสธด้วย JWT ผู้ใช้ · Auth hooks · อายุลิงก์อีเมล — ยังต้องทดสอบด้วยมือใน UAT (§ใบงานของ `pilot-uat-checklist.md`) |
...
|---|---|---|---|
| **U1** | `tools/check-canonical.mjs` | ข้อ 18 ระบุว่าต้องมีไฟล์นี้ | **ไม่เป็นช่องว่างแล้ว (23 ก.ย. 2569)** — ไฟล์มีจริง · `npm run check` ผ่านครบ จึง**นับเป็นเงื่อนไขเข้า UAT ได้** |
