...
**ราก** (`package.json`)
> **มาตรฐานของทีม: ฐานข้อมูลทดลองมีตัวเดียว** — พอร์ต `54329` เก็บที่ `.dev-db/`
> เปิดด้วย `npm run dev:db` เสมอ และ `web/.env.local` ต้องชี้ `JCRM_DEV_API_URL=http://127.0.0.1:54329`
>
> `JCRM_DEV_API_PORT` กับ `JCRM_DEV_DB_DIR` มีไว้เปิดฐานชั่วคราวตอนอยากทดลองเขียนข้อมูลโดยไม่กวนคนอื่น
> **ปิดและลบทิ้งเมื่อเสร็จ** อย่าปล่อยค้างไว้ ไม่งั้นจะเกิดกรณีที่เจ็บที่สุดคือ
> "ทดสอบผ่านบนฐานหนึ่ง แต่แอปต่ออยู่กับอีกฐานหนึ่ง" แล้วผลทดสอบที่ได้ไม่ใช่ของจริง
>
> **หลัง `git pull` ที่มี migration ใหม่ ต้องสั่ง `npm run dev:db:fresh`** — dev-api รัน migration
> เฉพาะตอนสร้างฐานใหม่เท่านั้น ถ้าไม่ล้าง ฟังก์ชันที่เพิ่งเพิ่มจะยังไม่มีในเครื่องคุณ
> และหน้าจอจะถอยไปใช้ค่าเริ่มต้นเงียบ ๆ โดยไม่มีใครรู้
