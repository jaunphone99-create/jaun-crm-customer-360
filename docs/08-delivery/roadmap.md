...
1. **ทุก visit ต้องปิดพร้อม outcome ในวันเดียวกัน** — ปิดไม่ทัน ระบบปิดให้เป็น `UNRECORDED` เวลา 00:05 และ **ไม่นับเป็นการบันทึกผลครบ** (ข้อ 4.1 · 4.2) กระทบ `OUTCOME_COMPLETION` (เกณฑ์ pilot ≥ 90% ตามข้อ 20.10 · เป้าองค์กร 95% ตามข้อ 12.3)
2. **ระบุตัวตนลูกค้าให้ได้** — `CAPTURE_RATE` วัด `IDENTIFIED_VISITS / VISITS` (เกณฑ์ pilot ≥ 80% แล้วค่อย ≥ 90% ตามข้อ 20.10 · เป้าองค์กร 95% ตามข้อ 12.3)
...
| P-3 | `CAPTURE_RATE` | `api.get_kpis` preset `LAST_30_DAYS` สาขา JP1 | **≥ 80%** ช่วงทดลองคู่กับกระดาษ · **≥ 90%** เมื่อใช้จริงเต็มสาขา (CANONICAL **ข้อ 20.10**) ต่อเนื่องตามระยะที่ตกลง **[รอยืนยัน]** |
| P-4 | `OUTCOME_COMPLETION` | เหมือน P-3 | **≥ 90%** (CANONICAL **ข้อ 20.10** "บันทึกผล visit ≥ 90%") |
...
> **เหตุที่ P-3 และ P-4 ไม่ใช่ 95%** — ข้อ 12.3 กำหนด 95% เป็น**เป้าขององค์กรหลังระบบและพนักงานนิ่งแล้ว**
> และเป็นค่าที่ `target_met` ของ `api.get_kpis` ใช้ หน้าจอจึงขึ้น "ต่ำกว่าเป้า" ได้ตามปกติระหว่าง pilot
> ส่วนเกณฑ์ที่ใช้**ตัดสิน pilot** คือ CANONICAL **ข้อ 20.10** ซึ่งเจ้าของโครงการอนุมัติไว้เป็นขั้น 80 → 90 → 95
> และข้อ 20 มีผลบังคับสูงสุด (ปรับ 23 ก.ย. 2569 · เดิมเอกสารนี้เขียน 95% ซึ่งขัดกับข้อ 20.10)
