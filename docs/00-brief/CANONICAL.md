...
> **ฉบับ v2.2 · 16 ก.ย. 2569** — ปรับจาก v1 ตามผลตรวจแบบ adversarial 5 มุม (148 ข้อ) · ตรวจความสอดคล้องภายใน v2 (22 ข้อ) · และข้อตัดสินจากผู้สร้างรอบแรก (ข้อ 19)
>
...
|---|---|---|
| Q1 | "ระบบ Login/สิทธิ์ที่กำลังพัฒนาอยู่" (A10) คือระบบใด และจะเป็นระบบกลางจริงหรือไม่ · พนักงานทุกคนมีรหัสพนักงาน HR หรือไม่ | CRM มี `core` ของตัวเอง ออกแบบให้ sync/แทนที่ได้ · `employee_code` NOT NULL (ห้ามซ้ำในบัญชีที่ไม่ใช่ DISABLED ตามข้อ 7.1) |
| Q2 | สาขา pilot | JP1 |
| Q3 | ทีมออนไลน์ส่วนกลางมีจริงหรือแอดมินสังกัดสาขา · การส่งต่อรายการจาก JPON ไปสาขาทำโดยใคร | มี `JPON` · ย้ายสาขาต้องมี assign ทั้งสองฝั่ง |
| Q4 | เวลาทำการของแต่ละสาขา | 10:00–21:00 |
| Q5 | บริษัทใช้ Google Workspace หรือ Microsoft 365 | รองรับทั้งสองแบบเฉพาะโดเมนที่อนุญาต |
| Q6 | แพ็กเกจ Supabase (Pro หรือ Team · lockout ด้วย hook ต้อง Team) และ region | Pro + PITR · Singapore |
| Q7 | เพดานการส่งออก (ข้อ 8.2) | ตามตาราง |
| Q8 | ระยะเวลาเก็บข้อมูล (ข้อ 10.3) และผู้ทำหน้าที่ DPO | ตามตาราง |
| Q9 | ข้อความประกาศความเป็นส่วนตัวฉบับ `PN-2026-01` | ต้องให้ฝ่ายกฎหมายร่าง |
| Q10 | `COMPARING` เป็นเหตุผลปิดหรือสถานะติดตาม | เหตุผลปิด |
| Q11 | ให้ Staff "เติมช่องที่ว่าง" ของลูกค้าที่ไม่ใช่ของตนได้หรือไม่ | ไม่ได้ |
| Q12 | SLA ตอบ Lead · รอคิว · opportunity ไม่เคลื่อนไหว | 30 นาที · 15 นาที · 7 วัน |
| Q13 | รายชื่อ sources · product types · tags · interaction types ที่ใช้จริง | ตามข้อ 5 |
| Q14 | ระบบ POS / ซ่อม / ผ่อน / สัญญา ที่มีอยู่ชื่ออะไร มี API หรือไม่ · รูปแบบเลขใบเสร็จ | `ref.source_systems` ตามข้อ 5.7 |
| Q15 | จำนวนบัญชี LINE OA / เพจ Facebook | รอยืนยัน |
| Q16 | ต้องเก็บเลขบัตรประชาชนใน CRM จริงหรือให้อยู่ระบบสัญญา | ไม่เก็บใน V1 |
| Q17 | JAUNPHONE กับ JAUN POWER MONEY เป็นนิติบุคคลเดียวกันหรือไม่ (ผลต่อ consent และการเปิดเผยข้อมูล) | ถือเป็น controller เดียว จนกว่ายืนยัน |
| Q18 | วันเริ่มสัปดาห์ · ปีบัญชี/ไตรมาส | จันทร์ · ไตรมาสปฏิทิน |
| Q19 | ต้องการให้ online visit เกิดเมื่อข้อความแรกของวัน หรือทุกบทสนทนาใหม่ | ข้อความแรกต่อช่องทางต่อวันธุรกิจ |
| Q20 | Phase 4 จะดึงข้อความ LINE/Meta อัตโนมัติหรือไม่ | รอยืนยัน |
| Q21 | นาที Lead Response นับนาทีปฏิทินหรือเวลาทำการ | นาทีปฏิทิน |
| Q22 | อนุญาตให้คนเดียวถือ MARKETING และ BUSINESS_ADMIN หรือไม่ | ไม่อนุญาต |
| Q23 | คำขอส่งออกของ EXECUTIVE ต้องให้ใครอนุมัติ | BUSINESS_ADMIN |
| Q24 | พนักงานที่ไม่มีอีเมลบริษัทจะรับคำเชิญและรีเซ็ตรหัสอย่างไร | ต้องมีอีเมล (ส่วนตัวได้) |
| Q25 | ใครถือสิทธิ์ Owner ของ Supabase organization (prod) | ≤ 2 คนที่ระบุชื่อ |
| Q26 | การลบข้อมูลตาม DSR ต้องมี BUSINESS_ADMIN ≥ 2 คน (ผู้ยืนยัน ≠ ผู้ดำเนินการ) · ยอมรับหรือให้ EXECUTIVE ยืนยันแทน | ต้องมี BA ≥ 2 คน |
| Q27 | ระบบภายนอกควรเก็บ `customer_no` หรือ uuid เป็นกุญแจลูกค้า | `customer_no` (ลูกค้าที่ถูกรวมชี้ไป survivor) |
| Q28 | SUPERVISOR ควรมอบ lead ที่ไม่มี owner ได้ (v2.2 อนุญาตในสาขาที่เป็นหัวหน้าทีม) | อนุญาต |
| Q29 | การแก้/ยกเลิก transaction ref ที่ผูกผิด | V1 เพิ่มได้อย่างเดียว · แก้ด้วยสคริปต์ BA |
| Q30 | ขอบเขตการลบใน backup หลัง anonymize (restore แล้วต้องทำซ้ำ) | เก็บรายการ `customer_no` + เวลา anonymize นอกฐานข้อมูล แล้วรันซ้ำหลัง restore |
