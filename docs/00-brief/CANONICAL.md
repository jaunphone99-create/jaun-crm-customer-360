...
| D53 | คำเชิญพนักงานเปิดใช้งานไม่ได้ เพราะไม่มีขั้นผูก `user_id` · และ `audit.login_events` ไม่มีทางเขียน | 7.1 · 9.5 · 19.3 · 20.13 | เพิ่ม `api.svc_link_invited_user` และ `api.svc_record_login_event` (service_role เท่านั้น) · มี test คุมใน `identity_acceptance.sql` |
| D54 | ค่าตั้งใน `app.settings` เปลี่ยนแล้วหน้าจอไม่เปลี่ยนตาม เพราะ `api.get_settings()` ต้องมี `settings.business`/`settings.system` ซึ่งพนักงานหน้าร้านไม่มี หน้าจอจึงฝังตัวเลขไว้ในโค้ด (`WAITING_LONG_MIN = 15` · ขอบ 30 วันของป้าย "ลูกค้าใหม่") | 11.2 · 20.14 | เพิ่ม `api.get_display_settings()` — อ่านอย่างเดียว · คืนเฉพาะค่าที่ใช้ **แสดงผล** (ไม่มีค่าที่ใช้ตัดสินสิทธิ์หรือเพดานอัตรา) จึงเปิดให้ทุกบัญชี ACTIVE อ่านได้ · `derived.new_customer_since` คิดขอบวันที่ฐานข้อมูลตามขอบเที่ยงคืน Asia/Bangkok (ข้อ 1.2) เพราะเบราว์เซอร์คิดเองไม่ได้ · นับรวมวันนี้ให้ตรงกับ preset `LAST_30_DAYS` เมื่อค่าตั้งเป็น 30 |
| D55 | สองในหกเหตุการณ์ที่เจ้าของโครงการสั่งให้ย้อนดูได้ — **เปิดเผยช่องทางติดต่อ** และ **ผูกลูกค้าเข้าสาขา** — ไม่มีทางอ่านผ่าน API เลย เพราะถูกบันทึกใน `audit.access_logs` ขณะที่ `api.search_audit` อ่านเฉพาะ `audit.audit_logs` และ `api.search_security_log` ไม่คืน access_logs ส่วน `crm.customer_branches` ก็ถูกยกเว้นจาก trigger audit | 9.5 · 20.14 | เพิ่ม `api.search_access_log(p)` — ต้องมี `audit.read` · **ไม่คืน `search_hashes`** เด็ดขาด (sha256 ไม่มี salt ของเบอร์มือถือไทยเดาย้อนกลับได้ในไม่กี่วินาที จะทำให้ตาราง audit กลายเป็นแหล่ง PII เสียเอง) คืนเป็น `search_term_count` แทน · มี test AU09–AU13 คุมไว้ |
| D56 | วิดเจ็ตหน้าหลักสามตัว (Funnel · แหล่งที่มา · เหตุผลที่ไม่สำเร็จ) ทำไม่ได้โดยไม่ให้หน้าจอหารเอง: ขั้น Opportunity ของ funnel ต้องเป็น `OPPORTUNITIES ÷ VISITS` (11.8%) ซึ่ง**ไม่มีรหัส KPI ใดคืนมา** (`OPPORTUNITY_RATE` หารด้วย `LEADS` = 41.3% คนละตัว) · โดนัทต้องการร้อยละของแต่ละช่องทางซึ่ง `p_group_by='CHANNEL'` ไม่คืน · และ `extra.lost_reasons` เดิมอยู่ใน `api.get_report` ที่ใช้ `report.view` ซึ่ง **STAFF ไม่มี** และ scope อาจไม่ตรงกับ `dashboard.view` ทำให้การ์ดกับกราฟบนหน้าจอเดียวกันคิดคนละขอบเขต | 12.1 · 14.7 · 20.15 | เพิ่ม `api.get_dashboard_charts(...)` ใช้สิทธิ์ `dashboard.view` เดียวกับการ์ด KPI · คืน funnel/channels/lost_reasons ที่จัดรูปแบบมาพร้อมแสดงทั้งหมด พร้อม `share`/`pct` เป็นสัดส่วน 0–1 ให้หน้าจอใช้วาดแท่งและส่วนโค้งโดยไม่ต้องหาร · ตรรกะ lost_reasons ย้ายไป `app.lost_reason_breakdown` ที่เดียว แล้วให้ `api.get_report` เรียกใช้ตัวเดียวกัน ไม่เขียน query ซ้ำสองที่ |
...
**ชุดที่ 1 · Core Flow หน้างาน** — ครบทั้ง 5 หน้า

| ลำดับ | หน้า | สถานะ | หมายเหตุ |
|---|---|---|---|
| 1 | เข้าสู่ระบบ (01) | 🟢 ใช้งานได้ | อีเมลหรือ `ST-NNNN` · ยืนยัน MFA · ลงทะเบียน TOTP · รับคำเชิญ/ตั้งรหัสผ่าน · ลืมรหัสผ่าน · `/auth/confirm` (ข้อ 20.13) |
...
**ชุดที่ 2 · หน้าจอผู้ดูแล** — ครบแล้ว (เหลือ 14 Master Data ที่ไม่อยู่ใน Phase 1)

| ลำดับ | หน้า | สถานะ | หมายเหตุ |
|---|---|---|---|
| 6 | ผู้ใช้งานและสิทธิ์ (13) | 🟢 ใช้งานได้ | ดึงมาทำก่อนเพราะงาน Identity ต้องใช้ · เชิญ · ปิดใช้งาน · บทบาท+สาขา |
| 7 | หน้าหลัก (02) | 🟢 ใช้งานได้ | การ์ด KPI ตามขอบเขตสิทธิ์ · Funnel · แหล่งที่มา · เหตุผลที่ไม่สำเร็จ · ตารางสาขา · กิจกรรมล่าสุด · ตัวเลขทุกตัวมาจาก RPC (ข้อ 20.17) |
| 8 | ศูนย์คุณภาพข้อมูล (12) | 🟢 ใช้งานได้ | 9 ชนิดปัญหา · แก้แล้วตัวนับลดจริงจาก RPC |
| 9 | ประวัติการใช้งาน (16) | 🟢 ใช้งานได้ | audit ธุรกิจ + ความปลอดภัย · ย้อนดูได้ครบหกเหตุการณ์ (D55) |
| 10 | ความเป็นส่วนตัว PDPA (17) | 🟢 ใช้งานได้ | DSR · ความยินยอม · legal hold · 2-person control ฐานข้อมูลบังคับ |
| 11 | ตั้งค่าระบบ (18) | 🟢 ใช้งานได้ | คอลัมน์ "มีผลกับ" · ป้าย มีผลจริง/บางส่วน/ยังไม่มีผล ตามความจริง (D54) |

หน้า 14 Master Data ไม่อยู่ใน Phase 1 (ข้อ 20.3) · หน้า 06 · 08 · 09 · 11 · 15 · 19 เป็น Phase 2–3
...
| Edge Functions (ข้อ 9.8) | 🟢 เขียนแล้ว 5 ตัว + `_shared` · ที่เหลือเป็น Phase 2 | ทีม Dev |
...
| รายชื่อ BUSINESS_ADMIN 2 คน · ผู้จัดการและพนักงาน pilot | รอยืนยัน — ต้องมีก่อนทดสอบ PDPA Success Flow | เจ้าของโครงการ (ข้อ 20.8 · 20.18) |
...
### 20.13 Identity & Access Completion — 23 ก.ย. 2569

งานรอบนี้ปิด Login · MFA · Invite · Account Lifecycle ก่อน Pilot ตามที่เจ้าของโครงการสั่ง

**เกณฑ์ยอมรับ 8 ข้อ** มีชุดทดสอบถาวรที่ `supabase/tests/identity_acceptance.sql` (48 assertion) จับคู่ 1:1 กับข้อที่สั่ง
ส่วนที่เป็นหน้าจอล้วน (redirect · การ์ดไม่มีสิทธิ์ · ล็อกอุปกรณ์ร่วม) พิสูจน์บนเบราว์เซอร์และระบุไว้ท้ายไฟล์ทดสอบ

**ของที่เพิ่มเข้า API (D53)**

| ฟังก์ชัน | ทำไมต้องมี |
|---|---|
| `api.svc_link_invited_user(p)` | `svc_prepare_invite` สร้างโปรไฟล์ INVITED โดยไม่ผูก `user_id` แต่ `activate_self()` ค้นด้วย `user_id = auth.uid()` — ขาดขั้นนี้คำเชิญเปิดใช้งานไม่ได้เลย · กัน: ต้องเป็น INVITED · ผูกซ้ำไม่ได้ · อีเมลต้องตรงกับที่เชิญ |
| `api.svc_record_login_event(p)` | `audit.login_events` ถูก REVOKE จากทุก role และไม่มีทางเขียน — ข้อกำหนด "Login / MFA / Invite / Disable Audit" จึงปิดไม่ครบ · เก็บตัวระบุเป็น sha256 เท่านั้น |

**ลำดับการปิดใช้งานบัญชี (สำคัญ)**
แอป **ห้าม** เรียก `api.disable_staff` เองก่อนแล้วค่อยเรียก Edge Function
เพราะเมื่อสถานะเป็น DISABLED แล้ว `svc_resolve_staff_code` จะหา `user_id` ไม่เจอ (คืนเฉพาะบัญชี ACTIVE)
การ ban ใน Auth จึงถูกข้ามเงียบ ๆ และระบบรายงานว่าสำเร็จทั้งที่เซสชันเดิมยังใช้งานต่อได้
**ลำดับที่ถูก:** Edge Function `disable-staff` ทำเองทั้งชุด — หา `user_id` → `api.disable_staff` ด้วย JWT ผู้กด → ban → `svc_finalize_disable`

**ยังเปิดอยู่ (ไม่บล็อกงาน dev · เป็น go-live dependency)**

| เรื่อง | ผู้รับผิดชอบ |
|---|---|
| Supabase project dev/staging/prod · `supabase/config.toml` | เจ้าของโครงการ/IT |
| ตั้งค่าอีเมลขาออก — ระหว่างนี้หน้าเชิญพนักงานแสดง**ลิงก์คำเชิญ**ให้ผู้จัดการคัดลอกส่งเอง | เจ้าของโครงการ/IT |
| รายชื่อ BUSINESS_ADMIN 2 คน (2-person control) | เจ้าของโครงการ |
| `api.svc_resolve_login_email` — ให้ `password-reset` ตรวจสถานะ ACTIVE จากอีเมลได้ (ตอนนี้พึ่ง ban ใน Auth) | ทีม Dev |
| เวลาตอบของ `staff-code-login` ยังไม่เท่ากันทุกเส้นทาง (ข้อความเท่ากันแล้ว) | ทีม Dev |

### 20.14 ชุดที่ 2 — Dashboard · คุณภาพข้อมูล · Audit · PDPA · ตั้งค่า (23 ก.ย. 2569)

**เกณฑ์ยอมรับ 5 ข้อที่เจ้าของโครงการสั่ง**

| # | เกณฑ์ | พิสูจน์ที่ |
|---|---|---|
| 1 | Dashboard ตัวเลขต้องตรงกับ API/DB ทุกตัว | ตัวเลขทุกตัวมาจาก `api.get_kpis` · หน้าจอไม่บวกลบเอง |
| 2 | แก้ Data Quality แล้วรายการและ Counter ต้องลดตามจริง | อ่าน `counts` จาก `api.list_data_quality_issues` ใหม่หลังแก้ · ไม่ลบแถวใน state |
| 3 | Audit ย้อนดู Reveal Contact · Link Branch · Invite · Disable · Merge · Edit สำคัญได้ | `api.search_audit` · `api.search_security_log` · `api.get_entity_history` |
| 4 | PDPA พิสูจน์ 2-person control จริง | ฐานข้อมูลเป็นผู้บังคับ ไม่ใช่หน้าจอ |
| 5 | Settings เปลี่ยน behavior ของระบบจริง | D54 · ดูตารางด้านล่าง |

**ค่าตั้งที่เปลี่ยนพฤติกรรมได้จริงแล้ว (เกณฑ์ข้อ 5)**

| คีย์ | มีผลกับ | พิสูจน์แล้ว |
|---|---|---|
| `sla.visitor_waiting_min` | ป้าย "รอนาน" ในคิวหน้ารับลูกค้า (07) | แก้ 15 → 5 แล้วพนักงาน `ST-0045` (aal1) อ่านค่าใหม่ได้ทันที |
| `badge.new_customer_days` | ป้าย "ลูกค้าใหม่" ในรายการลูกค้า (03) | แก้ 30 → 7 แล้ว `derived.new_customer_since` ขยับจาก 13 ส.ค. เป็น 5 ก.ย. (ขอบเที่ยงคืน Asia/Bangkok) |
| `session.shared_counter_idle_min` | เวลาล็อกหน้าจอของอุปกรณ์ร่วม | หน้าจอล็อกอ่านค่าจริงแทนค่าเริ่มต้นที่ฝังไว้ |

พนักงานที่ไม่มี `settings.business` แก้ค่าไม่ได้ — `api.update_setting` ตอบ
"ไม่มีสิทธิ์แก้ค่าตั้งนี้ (ต้องมี settings.business และยืนยัน MFA)" ตามเดิม

**ผลการตรวจรับชุดที่ 2 (Journey Test บนเบราว์เซอร์ + ฐานข้อมูลจริง)**

| # | เกณฑ์ | ผล |
|---|---|---|
| 1 | Dashboard ตัวเลขตรงกับ API/DB ทุกตัว | ✅ ทุกตัวเลขคือ `display` / `change_display` ที่ `api.get_kpis` จัดรูปแบบมาแล้ว · เวลาอ้างอิงมาจาก `clock` ของ RPC ไม่ใช่นาฬิกาเครื่อง · แถว "รวม" ของตารางสาขามาจาก `p_group_by='NONE'` คนละคำสั่ง ไม่ได้บวกจากแถวข้างบน (ลูกค้าคนเดียวที่ไปสองสาขานับไม่ซ้ำของทั้งสองสาขา บวกตรง ๆ จะเกินจริง) |
| 2 | แก้ Data Quality แล้วรายการและ Counter ลดตามจริง | ✅ รับทราบ visit ที่ยังไม่บันทึก 1 รายการ → ยอดลดจาก 28 เป็น 27 · JP2 จาก 8 เป็น 7 · ตัวนับอ่านใหม่จาก `counts`/`total` ของ RPC ทุกครั้ง ไม่มีที่ไหนลบแถวใน state ฝั่งเบราว์เซอร์ |
| 3 | Audit ย้อนดูหกเหตุการณ์ได้ | ✅ ครบทั้งหก หลังเพิ่ม `api.search_access_log` (D55) · พิสูจน์ด้วยการทำจริงแล้วเปิดดู: เปิดเบอร์ 2 · ผูกสาขา 1 · เชิญ 2 · ปิดบัญชี 2 · รวมลูกค้า 2 · แก้ข้อมูล 3 · ไม่มีค่าแฮชหลุดออกไปฝั่งเบราว์เซอร์สักมุมมองเดียว |
| 4 | PDPA พิสูจน์ 2-person control จริง | ✅ ฐานข้อมูลเป็นผู้บังคับ ไม่ใช่หน้าจอ — test `DS10` ปฏิเสธเมื่อผู้ดำเนินการเป็นคนเดียวกับผู้ยืนยัน · `DS14` ผ่านเมื่อเป็นผู้ดูแลข้อมูลธุรกิจอีกคน · หน้าจอบอกชื่อผู้ยืนยันและปิดปุ่มให้ตรงกับที่ฐานข้อมูลจะตอบ · **สาธิตเส้นทางที่สำเร็จบนหน้าจอยังทำไม่ได้** เพราะชุดข้อมูลตัวอย่างมี BUSINESS_ADMIN คนเดียว (เป็น go-live dependency เรื่องรายชื่อ 2 คน) |
| 5 | Settings เปลี่ยน behavior จริง | ✅ D54 · ตารางด้านบน |

### 20.15 Hardening ก่อน Pilot — 23 ก.ย. 2569

เจ้าของโครงการรับรองชุดที่ 2 แล้ว และสั่งทำ Hardening รอบสุดท้าย 1 รอบก่อนเปิด Pilot

**ที่สั่งไว้**

| # | งาน |
|---|---|
| 1 | Push `3ac8757` และ `4c1f6d8` ขึ้น remote |
| 2 | รวมฐานข้อมูลทดลองให้เหลือ instance เดียว แล้วใช้ตัวนั้นเป็นมาตรฐานของทีม |
| 3 | ปิดวิดเจ็ตหน้าหลักที่ยังขาด — Funnel · แหล่งที่มา · เหตุผลที่ไม่สำเร็จ · กิจกรรมล่าสุด |

**ขอบเขตที่อนุมัติเพิ่มสำหรับข้อ 3**
> อนุมัติให้แก้ API/RPC เพื่อเพิ่มค่าที่หน้าจอต้องใช้ได้
> แต่ยืนยันหลักเดิมว่า **ห้ามคำนวณ KPI หรือ Percentage ซ้ำที่ Frontend**
> ให้ Database/API เป็นผู้คำนวณและคืนค่าพร้อมแสดง

**มาตรฐานฐานข้อมูลทดลอง (ข้อ 2)**
หนึ่ง instance เท่านั้น: พอร์ต `54329` · โฟลเดอร์ `.dev-db/` · เปิดด้วย `npm run dev:db`
`JCRM_DEV_API_PORT` / `JCRM_DEV_DB_DIR` ใช้เปิดฐานชั่วคราวได้ แต่ต้องปิดและลบเมื่อเสร็จ
**หลัง `git pull` ที่มี migration ใหม่ ต้อง `npm run dev:db:fresh`** — dev-api รัน migration
เฉพาะตอนสร้างฐานใหม่ ไม่งั้นฟังก์ชันที่เพิ่งเพิ่มจะยังไม่มีในเครื่อง และหน้าจอจะถอยไปใช้ค่าเริ่มต้นเงียบ ๆ
(รายละเอียดใน `docs/08-delivery/environment-setup.md`)

### 20.16 Backlog — ค่าตั้งที่ยังไม่มีผลจริง (ไม่บล็อก Pilot)

เจ้าของโครงการสั่งให้คงสถานะ "ยังไม่มีผล" ไว้ตามจริง และบันทึกเป็น Backlog
หน้าตั้งค่า (18) ติดป้าย **ยังไม่มีผล** ให้สองแถวนี้อยู่แล้ว — ห้ามเปลี่ยนเป็น "มีผลจริง" จนกว่าจะทำเสร็จจริง

| คีย์ | ตั้งใจให้คุมอะไร | ทำไมยังไม่มีผล | ต้องทำอะไรจึงจะมีผล |
|---|---|---|---|
| `security.login_ip_per_15min` | จำนวนครั้งที่ลองเข้าสู่ระบบจาก IP เดียวกันใน 15 นาที | Edge Function อ่านเพดานจากตัวแปรสภาพแวดล้อม `LOGIN_IP_PER_15MIN` ไม่ได้อ่าน `app.settings` | ให้ `supabase/functions/_shared/rate-limit.ts` อ่านค่าจาก `app.settings` แทน (ระวัง: ด่านนี้ทำงานก่อนรู้ว่าผู้ใช้เป็นใคร จึงต้องอ่านด้วย service_role และ cache สั้น ๆ ไม่ให้ยิงฐานข้อมูลทุก request) |
| `allowed_sso_domains` | โดเมนอีเมลที่เข้าสู่ระบบด้วย SSO ได้ | ยังไม่ได้เปิด SSO เลย (ข้อ 9.2.1 · D23 · รอยืนยัน Q5) ระบบปัจจุบันใช้รหัสพนักงาน/อีเมล + รหัสผ่าน | เปิด SSO ก่อน แล้วค่อยให้ขั้นตอนเข้าสู่ระบบตรวจโดเมนจากค่านี้ |

### 20.17 ผล Hardening และสถานะ Pilot — 23 ก.ย. 2569

**ข้อ 1 · Push** ✅ `71ba7cd..4c1f6d8` และ commit ของรอบ Hardening ขึ้น `main` ครบ

**ข้อ 2 · ฐานข้อมูลทดลองหนึ่งตัว** ✅ เหลือ `54329` / `.dev-db/` ตามข้อ 20.15 · ปิดและลบตัวชั่วคราวแล้ว

**ข้อ 3 · วิดเจ็ตหน้าหลัก** ✅ ครบทั้งสี่ตัว

| วิดเจ็ต | ที่มาของตัวเลข | ตรวจแล้ว |
|---|---|---|
| Funnel ลูกค้า | `api.get_dashboard_charts.funnel` | 3,125 (100.0%) → 892 (28.5%) → 368 (11.8%) → 215 (6.9%) ตรงข้อ 13.1 |
| แหล่งที่มาลูกค้า | `…channels` | กลางวง 1,284 = การ์ด `UNIQUE_CUSTOMERS` · ผลรวมทุกส่วน = 1,284 |
| เหตุผลที่ไม่สำเร็จ | `…lost_reasons` | 5 อันดับ + อื่น ๆ · ร้อยละรวม 100.0% ตรงข้อ 13.4 |
| กิจกรรมล่าสุด | `crm.customers` ผ่าน RLS | 5 แถว เรียงตาม `last_activity_at` · ไม่มีคอลัมน์ช่องทางติดต่อ |

การมองเห็นตามบทบาทตรงข้อ 14.7 ทุกช่อง: MARKETING ไม่เห็นกิจกรรมล่าสุด (ไม่มี `customer.read`) ·
SUPERVISOR/STAFF ไม่เห็นกราฟทั้งสาม · SYSTEM_ADMIN ได้การ์ดไม่มีสิทธิ์

**Full Gate** — `npm run check` ผ่าน (canonical · prototype · tsc+lint · `db:test` 20 ไฟล์) · `next build` ผ่าน 18 เส้นทาง
**Smoke Test** — `npm run smoke` ผ่านทั้งหมด · เส้นทาง `Login → รับลูกค้า → Quick Capture → Customer 360 → Dashboard → Audit`
เก็บเป็นสคริปต์ถาวรที่ `tools/smoke-test.py` ให้รันซ้ำได้ทุกครั้งก่อนปล่อยของ

### 20.18 สถานะ: **Pilot Ready — JAUNPHONE 1**

**สิ่งที่ยังเปิดอยู่ และไม่บล็อก Pilot**

| เรื่อง | สถานะ | ผู้รับผิดชอบ |
|---|---|---|
| Supabase project dev/staging/prod · `supabase/config.toml` | go-live dependency | เจ้าของโครงการ/IT |
| ตั้งค่าอีเมลขาออก — ระหว่างนี้หน้าเชิญพนักงานแสดงลิงก์ให้คัดลอกส่งเอง | go-live dependency | เจ้าของโครงการ/IT |
| BUSINESS_ADMIN คนที่ 2 — ต้องมีก่อนทดสอบ PDPA Success Flow | เจ้าของโครงการยืนยันก่อน go-live | เจ้าของโครงการ |
| `security.login_ip_per_15min` · `allowed_sso_domains` ยังไม่มีผล | Backlog ข้อ 20.16 · หน้าจอติดป้าย "ยังไม่มีผล" ตามจริง | ทีม Dev |
| `api.svc_resolve_login_email` · เวลาตอบของ `staff-code-login` | Backlog ข้อ 20.13 | ทีม Dev |

**ส่วนที่ตั้งใจไม่ทำในรอบนี้**

`ผลงานรายพนักงาน` (W4 ของชุดสาขา) — ข้อ 14.7 ระบุว่า BRANCH_MANAGER ควรเห็นตารางนี้แทนตารางสาขา
แต่ `sitemap-screen-specs` ข้อ 5.5 ระบุชัดว่าเป็น **Phase 3** และตารางตามข้อ 13.6 มี 10 แถว
รวมคอลัมน์ "งาน วันนี้ · เกินกำหนด" ที่ผูกกับหน้างาน (08) ซึ่งเป็น Phase 2
