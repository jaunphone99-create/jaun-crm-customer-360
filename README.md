...
`dev:db` ใช้เวลาจริงสำหรับคิววันนี้และรายงาน ทั้งเมื่อสร้างฐานข้อมูลใหม่และเปิดฐานเดิม
ข้อมูลตัวอย่างยังคงวันที่เดิม หากต้องการดูรายงาน snapshot ของชุดตัวอย่างโดยเฉพาะ ใช้
`npm run dev:db -- --as-of 2026-09-11T10:24:00+07:00` (รายการที่สร้างใหม่ยังใช้เวลาจริง)
กลับมาใช้เวลาจริงด้วยการเปิด `npm run dev:db` ตามปกติ โดยไม่ต้องล้างข้อมูล
ชุดทดสอบ `npm run db:test` ยังคงตรึงเวลาตาม seed เพื่อให้ผลซ้ำได้
